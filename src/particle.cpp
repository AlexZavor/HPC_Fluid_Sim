...
#define SCHEDULE schedule(dynamic, 8)
...
void particle_update(particle* particles, int size, double dt, input_t* input){

	#ifdef __NVCC__
	// call kernal here. run function
	#else

	/*
	*
	*  Code from a neat example from:
	*	TODO: Credit
	*
	*/
	...
	for(int i=0; i < size; ++i)
	{
		...
		if(input->mouseLeft||input->mouseRight){ // Magic mouse
			if( attr_dist2 < attr_l*attr_l )
				particles[i].force -= (particles[i].pos - mouse_vect) / 512;// * (input->mouseRight?-1:1);
		}
